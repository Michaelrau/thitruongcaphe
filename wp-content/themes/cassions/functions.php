...
add_action( 'wp_router_generate_routes', 'wp_add_cn_routes', 20 );

function wp_add_cn_routes( $router ) { 
    $route_args = array('path' => '^commodities-news','page_callback' => 'cn_route_callback','title' => __( 'Commodities News' ),
                        'template' => array('commodities_news.php', dirname( __FILE__ ) . '/wp-route-pages/commodities_news.php'));
    $router->add_route( 'commodities-news', $route_args );
}	

function cn_route_callback(  ) {
    echo '';
}

add_action( 'wp_router_generate_routes', 'wp_add_tt_routes', 20 );

function wp_add_tt_routes( $router ) { 
    $route_args = array('path' => '^tin-tuc','page_callback' => 'tt_route_callback','title' => __( 'Tin tức' ),
                        'template' => array('news.php', dirname( __FILE__ ) . '/wp-route-pages/news.php'));
    $router->add_route( 'tin-tuc', $route_args );
}	

function tt_route_callback(  ) {
    echo '';
}

add_action( 'wp_router_generate_routes', 'wp_add_ptdb_routes', 20 );

function wp_add_ptdb_routes( $router ) { 
    $route_args = array('path' => '^phan-tich-du-bao','page_callback' => 'ptdb_route_callback','title' => __( 'Phân tích dự báo' ),
                        'template' => array('analyze.php', dirname( __FILE__ ) . '/wp-route-pages/analyze.php'));    
    $router->add_route( 'phan-tich-du-bao', $route_args );
}

function ptdb_route_callback(  ) {
    echo '';
}

add_action( 'wp_router_generate_routes', 'wp_add_bgtt_routes', 20 );

function wp_add_bgtt_routes( $router ) { 
    $route_args = array('path' => '^bang-gia-truc-tuyen','page_callback' => 'bgtt_route_callback','title' => __( 'Bảng giá trực tuyến' ),
                        'template' => array('online_prices.php', dirname( __FILE__ ) . '/wp-route-pages/online_prices.php'));    
    $router->add_route( 'bang-gia-truc-tuyen', $route_args );
}

function bgtt_route_callback(  ) {
    echo '';
}

add_action( 'wp_router_generate_routes', 'wp_add_vhvn_routes', 20 );

function wp_add_vhvn_routes( $router ) { 
    $route_args = array('path' => '^van-hoa-van-nghe','page_callback' => 'vhvn_route_callback','title' => __( 'Văn hóa - Văn nghệ' ),
                        'template' => array('cultural.php', dirname( __FILE__ ) . '/wp-route-pages/cultural.php'));
    $router->add_route( 'van-hoa-van-nghe', $route_args );
}

function vhvn_route_callback(  ) {
    echo '';
}
